...
import { getBets, deleteBetById } from 'apiSdk/bets';
import { BetInterface } from 'interfaces/bet';

type ColumnType = ColumnDef<BetInterface, unknown>;

interface BetListPageProps {
...
export function BetListPage(props: BetListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BetInterface>>(
    () => `/bets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BetInterface) => {
    if (hasAccess('bet', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/bets/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/bets/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('bet', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/bets/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('bet', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="bet" />
        </Flex>

        {hasAccess('bet', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/bets/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
